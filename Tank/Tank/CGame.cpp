...
void CGame::GameRunDraw()
{
	...
	CBitmap bmp;
	bmp.CreateCompatibleBitmap(dc, rc.Width(), rc.Height());  //该函数用于创建与指定的设备环境相关的设备兼容的位图
	...
	::BitBlt(hdc, 0, 0, rc.Width(), rc.Height(), m_dcMemory.GetSafeHdc(), 0, 0, SRCCOPY);// 复制到屏幕
	dc->DeleteDC();                                       // 释放
	return;
}
...
void CGame::DrawFps(Graphics &gh)
{
	...
	// 输出fps
	{
		...
		PointF origin(static_cast<float>(rc.right - 50), static_cast<float>(rc.top + 2));  // 右上角显示
